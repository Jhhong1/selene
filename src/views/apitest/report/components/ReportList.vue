...
export default {
  ...
  methods: {
    taskList () {
      this.$api.api.getTaskList(1, 1000, this.projectName).then(
        (response) => {
    ...
    change (val) {
      this.$api.api.resultStatics(val, this.projectName).then(
        (response) => {
          ...
          if (ret.not_run > 0) {
            subdata.push(
              { value: ret.not_run, name: '未执行', itemStyle: { color: '#909399' } }
            )
          }
        }
      ).catch(
