  ...
  <div>
    ...
    <el-form :model="addCase" ref="addCase" :rules="rules" class="case-form">
      ...
      <el-form-item label="头部信息" :label-width="formLabelWidth" prop="headers">
        <input-counter v-model="addCase.headers"></input-counter>
      </el-form-item>
...
export default {
  ...
  methods: {
    ...
    caseDetail () {
      ...
      this.$api.api.getCaseDetail(caseId).then(
        (response) => {
    ...
    updateCase (params) {
      let caseId = this.$route.params.id
      this.$api.api.updateCase(caseId, params).then(
        (response) => {
          this.$message({
            showClose: true,
            message: '更新用例成功',
    ...
    filterCase () {
      ...
      for (let i = 0, len = ret.length; i < len; i++) {
        let ca = ret[i]
        try {
          if (ca.setups.length || ca.teardowns.length) {
            ret.splice(i, 1)
          }
        } catch (e) {}
      }
      let currentCase = this.$route.params.id
      for (let i = 0, len = ret.length; i < len; i++) {
        try {
          let ca = ret[i]
          if (ca.id.toString() === currentCase) {
            ret.splice(i, 1)
          }
        } catch (e) {}
      }
