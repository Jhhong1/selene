  ...
  <div>
    ...
    <el-form :model="addCase" ref="addCase" :rules="rules" class="case-form">
      ...
      <el-form-item label="变量" :label-width="formLabelWidth" prop="variables">
        <input-counter v-model="addCase.variables" size="mini"></input-counter>
      </el-form-item>
      ...
      <el-form-item label="头部信息" :label-width="formLabelWidth" prop="headers">
        <input-counter v-model="addCase.headers" size="mini"></input-counter>
      </el-form-item>
      <el-form-item label="请求内容" :label-width="formLabelWidth" prop="body">
        <vue-json-editor v-model="addCase.body" mode="code"></vue-json-editor>
      </el-form-item>
      <el-form-item label="结果断言" :label-width="formLabelWidth" prop="asserts">
        <asserts v-model="addCase.asserts"></asserts>
      </el-form-item>
      <el-form-item label="提取参数" :label-width="formLabelWidth" prop="extracts">
        <extracts v-model="addCase.extracts"></extracts>
      </el-form-item>
      ...
      <el-form-item label="失败后是否继续" :label-width="formLabelWidth" prop="continues">
        <el-tooltip :content="addCase.continues" placement="right-start" class="el--swith">
          <el-switch v-model="addCase.continues" active-color="#13ce66" inactive-color="#dedfe0" active-value="true" inactive-value="false"></el-switch>
        </el-tooltip>
      </el-form-item>
...
export default {
  ...
  data () {
    ...
    return {
      ...
      rules: {
        name: [
          { validator: validateCaseName, required: true, trigger: 'blur' }
        ],
        url: [
          { validator: validateUrl, required: true, trigger: 'blur' }
        ],
        method: [
          { validator: validateMethod, required: true, trigger: 'blur' }
        ]
      }
    }
  },
  methods: {
    ...
    create (params) {
      this.$api.api.createTestCase(params).then(
        (response) => {
          this.$message({
            showClose: true,
            message: '添加用例成功',
    ...
    filterCase () {
      let ret = this.apiCaseLists
      if (ret) {
        for (let i = 0, len = ret.length; i < len; i++) {
          let val = ret[i]
          if (val.setups.length || val.teardowns.length) {
          } else {
            this.casesDate.push(val)
          }
        }
      }
    }
  ...
  computed: {
    apiCaseLists () {
      if (this.$store.state.apiCaseLists.length === 0) {
        let res = JSON.parse(window.localStorage.getItem('apiCaseLists'))
        this.$store.commit('STORE_API_CASES', res)
      }
      return this.$store.state.apiCaseLists
    }
  },
  mounted () {
    // remove precondition and post condition
    // this.filterCase()
  }
}
</script>
