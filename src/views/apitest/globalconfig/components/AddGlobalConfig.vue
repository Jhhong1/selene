  ...
  <div>
    ...
    <el-form :model="configForm" ref="configForm" :rules="rules" class="case-form">
      ...
      <el-form-item label="头部信息" :label-width="formLabelWidth" prop="headers">
        <input-counter v-model="configForm.headers" size="mini"></input-counter>
      </el-form-item>
      <el-form-item label="变量" :label-width="formLabelWidth" prop="variables">
        <input-counter v-model="configForm.variables" size="mini"></input-counter>
      </el-form-item>
      <el-form-item label="设为全局变量" :label-width="formLabelWidth" prop="globalConfig">
        <el-tooltip :content="configForm.globalConfig" placement="right-start" class="el--swith">
          <el-switch v-model="configForm.globalConfig" active-color="#13ce66" inactive-color="#dedfe0" active-value="true" inactive-value="false"></el-switch>
        </el-tooltip>
      </el-form-item>
...
export default {
  ...
  methods: {
    ...
    create (params, project, formName) {
      this.$api.api.createConfig(params, project).then(
        (response) => {
          this.$message({
            showClose: true,
            message: '添加配置成功',
