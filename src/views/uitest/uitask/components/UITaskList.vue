<template>
    <div>
        <router-link
            tag="el-button"
            :to="{ name: 'AddUITask', query: $route.query }"
            ...
            <el-table-column label="名称" min-width="200">
                <template slot-scope="scope">
                    <ul class="ul-style">
                        <li>
                            <router-link
                                :to="{ name: 'UITaskDetail', params: { name: scope.row.name, id: scope.row.id }, query: $route.query }"
    ...
    methods: {
        ...
        handleCommand(command) {
            if (command.type === 'del') {
                ...
                this.$router.push({ name: 'UITaskDetail', params: { name: command.name, id: command.row }, query: this.$route.query })
            } else if (command.type === 'exec') {
                this.choices(command.row)
            } else if (command.type === 'update') {
                this.$router.push({ name: 'UpdateUITask', params: { id: command.row }, query: this.$route.query })
            }
        },
        clearSelect() {
            this.taskId = null
