    ...
    <div class="el-bread">
        ...
        <el-tabs v-model="activeName" style="background-color: white">
            ...
            <el-tab-pane label="测试集" name="cases">
                ...
                <!--执行测试任务,选择label-->
                <el-dialog title="执行" :close-on-click-modal="false" :visible.sync="labelDialog" :before-close="clearSelect" class="dialog-header">
            ...
            <el-tab-pane label="历史记录" name="history">
                <div style="padding-left: 20px; padding-right: 20px">
                    <record v-model="taskHistory" :categories="category" :kind="kind"></record>
                </div>
            </el-tab-pane>
        </el-tabs>
    </div>
...
export default {
    ...
    methods: {
        ...
        remove(id) {
            this.$api.api
                ...
                .then(() => {
                    ...
                    this.$router.push({ name: 'UITaskList', query: this.$route.query })
                })
                .catch(error => {
        ...
        infoHandleCommand(command) {
            if (command === 'update') {
                this.$router.push({ name: 'UpdateUITask', params: { id: this.taskId }, query: this.$route.query })
